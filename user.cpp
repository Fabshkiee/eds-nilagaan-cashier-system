...
void loadUsers() {
  ...
  if (userCount == 0) { // default accounts if wala accounts
...
void addCashier() {
  ...
  cout << "Enter username (or type 'exit' to cancel): ";
  cin >> username;
  if (username == "exit") {
    clearSystem();
...
void showAllCashiers() {
  ...
  for (int i = 0; i < userCount; i++) {
    if (!users[i].isAdmin) {
      cout << i << ". Username: " << users[i].username << endl;
      found = true;
    }
  }
...
void updateCashier() {
  ...
  string username;
  cout << "Enter username to update (or type 'exit' to cancel): ";
  cin >> username;
  if (username == "exit") {
    clearSystem();
    cout << "Returning to user management.\n";
...
void deleteCashier() {
  ...
  string username;
  cout << "Enter username to delete (or type 'exit' to cancel): ";
  cin >> username;
  if (username == "exit") {
    clearSystem();
    cout << "Returning to user management.\n";
